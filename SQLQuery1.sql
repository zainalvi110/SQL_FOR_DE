...
select * from orders_data order by order_date asc,quantity desc
select profit/sale as ratio from orders_data 
select category,sum(sale)as total_sales from orders_data where region = 'central' and city = 'Houston'
group by category
order by total_sales

select distinct category from orders_data
select * from orders_data where quantity between 2 and 5;
select distinct city from orders_data where city in(select city from orders_data where sale<=10)
select * from orders_data where order_date >='2020-01-01' order by order_date asc 
select * from orders_data
select count(*) as total_furniture_sales  from orders_data where category='Furniture'

select * from orders_data where customer_name like '______a___ff%'
-- _ underscore ek charecter ko represent kr raha idhr
select * from orders_data where customer_name like '_[leu]%' -- mtlb in teeno mn se jo bhi hay second charecter wo sub le aao
select sum(sale) as total_sales from orders_data
select count(sale) from orders_data
select count(*) from orders_data where sale<200
SELECT category, COUNT(*) AS low_sale_count
FROM orders_data
WHERE sale < 200
GROUP BY category;

select city ,count(*) from orders_data where order_date between'2020-01-01'and '2021-01-01' and sale < 200 
group by city
...
WHERE order_date >= '2020-01-01' 
  ...
  AND sale < 200;
  update orders_data set customer_name = Null where order_id in('CA-2020-138688','CA-2020-137330')
  select * from orders_data where customer_name is null
  select category,sum(sale)as sum_sales,AVG(profit)as avg_profit from orders_data group by category


  select category,sum(sale) from orders_data
  group by category
select category, sum(sale)as total_sales,count(customer_name)as customer_reoccurance from orders_data
group by category
having count(customer_name)>1
order by category

SELECT customer_name, COUNT(*) AS order_count, SUM(sale) AS total_sales
FROM orders_data
...
SELECT  customer_name,category, COUNT(*) AS order_count, SUM(sale) AS total_sales
FROM orders_data
...
select * from orders_data where customer_name='Brosina Hoffman'
...
select r.return_reason,o.category,sum(o.sale) as return_sale from orders_data o
inner join returns_data r
on o.order_id=r.order_id
group by return_reason,category
order by category,return_reason,return_sale asc
